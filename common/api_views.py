...
from products.models import StockOut, Product
...
class DailySalesAPI(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for day in range(12):
            ...
            retailer_sales = (
                Invoice.objects.filter(date__icontains=sales_day.date())
            )

            retailer_products = (Product.objects.all().values_list('id', flat=True))
        ...
        for week in range(1, 13):
            ...
            retailer_sales = (
                Invoice.objects.filter(
                    date__gte=sales_start_week,
                    date__lt=sales_end_week
                )
            )
            retailer_products = (Product.objects.all().values_list('id', flat=True))
...
class MonthlySalesAPI(DailySalesAPI):
    def get(self, request, *args, **kwargs):
        ...
        for month in range(12):
            ...
            retailer_products = (Product.objects.all().values_list('id',
                                                                    flat=True))
            stockout = StockOut.objects.filter(
